...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # Save run settings  save hyp&opt
    ...
    init_seeds(2 + rank)  # random seeds
    with open(opt.data) as f:
        data_dict = yaml.load(f, Loader=yaml.FullLoader)  # data dict
    with torch_distributed_zero_first(rank):  # synchronize the progress
        check_dataset(data_dict)  # check the dataset,if dataset is none,download
    ...
    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups  bn weights bias
    for k, v in model.named_modules():
        if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):
            pg2.append(v.bias)  # biases
        if isinstance(v, nn.BatchNorm2d):   # BN
            pg0.append(v.weight)  # no decay
        elif hasattr(v, 'weight') and isinstance(v.weight, nn.Parameter):  # weights
            pg1.append(v.weight)  # apply decay

    if opt.adam:  # pg0 bn optimizer
    ...
    if pretrained:
        # Optimizer
        # load optimizer and best_fitness
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.image_weights:    # initial false
            # Generate indices
            if rank in [-1, 0]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/wangzhe.yaml', help='data.yaml path')
    ...
    if opt.global_rank in [-1, 0]:
        check_git_status() # check the latest code
        check_requirements()
